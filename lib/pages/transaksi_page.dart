...
class _TransaksiPageState extends State<TransaksiPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error loading transaksi: $e')));
      }
    }
  }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              'Apakah Anda yakin ingin menghapus transaksi ${transaksi['no_invoice']}?',
            ),
            actions: [
              TextButton(
  ...
  String _getCustomerDisplayName(Map<String, dynamic> transaksi) {
    ...
    if (transaksi['contact_source'] == 'contact' &&
        transaksi['contact_data'] != null) {
      try {
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    onPressed:
                                        () =>
                                            _showDeleteConfirmation(transaksi),
                                    icon: Icon(
...
class _TransaksiFormPageState extends State<TransaksiFormPage> {
  ...
  bool _hideDataBarangInPdf =
      false; // Checkbox state for hiding data barang in PDF
  ...
  Future<void> _loadTransaksi() async {
    ...
    try {
      ...
      if (transaksi != null) {
        ...
        if (transaksi.contactSource == 'contact' &&
            transaksi.contactData != null) {
        ...
        final details = await _databaseHelper.getTransaksiDetails(
          widget.transaksiId!,
        );
        _detailItems =
            details
                .map(
                  (detail) => TransaksiDetailItem(
                    qty: detail.qty,
  ...
  Future<Invoice> _createInvoiceFromTransaksi(int transaksiId) async {
    ...
    print(
      'DEBUG: Transaksi dari database - ID: $transaksiId, customerId: ${transaksi.customerId}',
    );
    print(
      'DEBUG: Transaksi data - noInvoice: ${transaksi.noInvoice}, status: ${transaksi.status}',
    );
    print(
      'DEBUG: Transaksi barang - merk: ${transaksi.merkBarang}, sn: ${transaksi.snBarang}, nama: ${transaksi.namaBarang}',
    );
    print(
      'DEBUG: Contact source: ${transaksi.contactSource}, Contact data: ${transaksi.contactData}',
    );
    ...
    if (transaksi.contactSource == 'contact' && transaksi.contactData != null) {
      try {
        ...
        print(
          'DEBUG: Contact dari kontak HP: ${contactInfo.name} - ${contactInfo.phoneNumber}',
        );
      } catch (e) {
    ...
    if (contactInfo == null && transaksi.customerId != null) {
      ...
      print(
        'DEBUG: Customer dari database: ${customer?.nama} (ID: ${customer?.id})',
      );
    }

    if (contactInfo == null && customer == null) {
      print(
        'DEBUG: Tidak ada customer atau contact data, menggunakan Customer Umum',
      );
    }

    // Company info - load from database settings
    final companySettings = await _databaseHelper.getCompanyInfo();
    final company = CompanyInfo(
      name: companySettings?.companyName ?? 'Lentera Komputer',
      address:
          companySettings?.companyAddress ??
          'Jl. Contoh No. 123\nKota, Provinsi 12345',
      phone: companySettings?.companyPhone ?? '0821-1712-3434',
      email: companySettings?.companyEmail ?? 'lenteracomp.bdg@gmail.com',
      website: companySettings?.companyWebsite ?? 'www.lenterakomputer.com',
    );
    ...
    final invoiceItems =
        details.map((detail) {
          print(
            'DEBUG: Detail item - qty: ${detail.qty}, desc: ${detail.deskripsi}, harga: ${detail.harga}',
          );
          return InvoiceItem(
            ...
            namaBarang:
                detail.namaBarang?.trim().isEmpty == true
                    ? null
                    : detail.namaBarang?.trim(),
            snBarang:
                detail.snBarang?.trim().isEmpty == true
                    ? null
                    : detail.snBarang?.trim(),
            garansi:
                detail.garansi?.trim().isEmpty == true
                    ? null
                    : detail.garansi?.trim(),
          );
        }).toList();
    ...
    final calculatedSubtotal = invoiceItems.fold(
      0.0,
      (sum, item) => sum + item.amount,
    );
    print(
      'DEBUG: Calculated subtotal: $calculatedSubtotal, DB subtotal: ${transaksi.subtotal}',
    );

    final invoice = Invoice(
      invoiceNumber: transaksi.noInvoice,
      date: transaksi.tanggal,
      dueDate: transaksi.tanggal.add(
      ...
      merkBarang:
          transaksi.merkBarang?.trim().isEmpty == true
              ? null
              : transaksi.merkBarang?.trim(),
      snBarang:
          transaksi.snBarang?.trim().isEmpty == true
              ? null
              : transaksi.snBarang?.trim(),
      namaBarang:
          transaksi.namaBarang?.trim().isEmpty == true
              ? null
              : transaksi.namaBarang?.trim(),
      dpAmount: transaksi.dp > 0 ? transaksi.dp : null,
      paymentStatus: transaksi.status,
      paymentMethod:
          transaksi.metodePembayaran?.trim().isEmpty == true
              ? 'Transfer Bank'
              : transaksi.metodePembayaran?.trim() ?? 'Transfer Bank',
    );

    print(
      'DEBUG: Invoice created - hasDP: ${invoice.hasDP}, sisaAmount: ${invoice.sisaAmount}',
    );
  ...
  Future<void> _generatePdf() async {
    ...
    try {
      ...
      final pdfFile = await PdfService.generateInvoicePdf(
        invoice,
        hideDataBarang: _hideDataBarangInPdf,
      );
  ...
  Future<void> _saveTransaksi() async {
    ...
    for (int i = 0; i < _detailItems.length; i++) {
      if (!_detailItems[i].isValid) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Item ${i + 1} tidak lengkap')));
        return;
      }
    }
    ...
    try {
      ...
      print(
        'DEBUG: Selected customer: ${_selectedCustomer?.nama} (ID: ${_selectedCustomer?.id})',
      );

      // Validasi customer jika dipilih
      if (_selectedCustomer != null) {
        final customerExists = await _databaseHelper.getCustomer(
        ...
        if (customerExists == null) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Customer yang dipilih tidak valid. Silakan pilih customer lain.',
                ),
              ),
            );
          }
      ...
      String? contactData;
      if (_customerSource == CustomerSource.contact &&
          _selectedContact != null) {
        contactSource = 'contact';
        contactData = jsonEncode(_selectedContact!.toJson());
      } else if (_customerSource == CustomerSource.database &&
          _selectedCustomer != null) {
      ...
      final transaksi = Transaksi(
        ...
        status: status,
        metodePembayaran:
            _metodePembayaranController.text.trim().isEmpty
                ? null
                : _metodePembayaranController.text.trim(),
        merkBarang:
            _merkBarangController.text.trim().isEmpty
                ? null
                : _merkBarangController.text.trim(),
        snBarang:
            _snBarangController.text.trim().isEmpty
                ? null
                : _snBarangController.text.trim(),
        namaBarang:
            _namaBarangController.text.trim().isEmpty
                ? null
      ...
      if (widget.transaksiId == null) {
        ...
        final freshInvoiceNumber =
            await _databaseHelper.generateInvoiceNumber();
        ...
        final existingDetails = await _databaseHelper.getTransaksiDetails(
          transaksiId,
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error saving transaksi: $e')));
      }
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> _generateInvoiceForDate(DateTime date) async {
    try {
      final newInvoiceNumber = await _databaseHelper
          .generateInvoiceNumberForDate(date);
      if (mounted) {
        setState(() {
                      ...
                      children: [
                        ...
                        if (_hideDataBarangInPdf) ...[
                          Container(
                            ...
                            child: TextFormField(
                              initialValue:
                                  item.namaBarang ?? _namaBarangController.text,
                              decoration: InputDecoration(
                                labelText: 'Nama Barang',
                                border: InputBorder.none,
                                contentPadding: const EdgeInsets.all(16),
                            ...
                            Expanded(
                              child: Container(
                                ...
                                child: DropdownButtonFormField<DeskripsiSource>(
                                  ...
                                  items:
                                      DeskripsiSource.values.map((source) {
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            if (item.deskripsiSource ==
                                DeskripsiSource.ketikBebas) ...[
                              Container(
                                decoration: BoxDecoration(
                                  color: colorScheme.surfaceContainerHighest
                                      .withValues(alpha: 0.3),
                            ...
                            ] else if (item.deskripsiSource ==
                                DeskripsiSource.dariBarang) ...[
                              Container(
                                decoration: BoxDecoration(
                                  color: colorScheme.surfaceContainerHighest
                                      .withValues(alpha: 0.3),
                              ...
                              Container(
                                ...
                                child: DropdownButtonFormField<Service>(
                                  ...
                                  items:
                                      _serviceList.map((service) {
                                        return DropdownMenuItem(
                                          value: service,
                                          child: Text(
                                            service.jenisKerusakan.join(', '),
                                          ),
                                        );
                                      }).toList(),
                                  onChanged: (value) {
                          ...
                          children: [
                            ...
                            Expanded(
                              ...
                              child: Container(
                                ...
                                child: TextFormField(
                                  initialValue: NumberUtils.formatToThousands(
                                    item.harga,
                                  ),
                                  decoration: InputDecoration(
                                    labelText: 'Harga',
                                  ...
                                  onChanged: (value) {
                                    item.harga = NumberUtils.parseFromThousands(
                                      value,
                                    );
                                    setState(() {});
                                  },
                        ...
                        if (_hideDataBarangInPdf) ...[
                          ...
                          Container(
                            ...
                            child: TextFormField(
                              initialValue:
                                  item.snBarang ?? _snBarangController.text,
                              decoration: InputDecoration(
                                labelText: 'SN (Serial Number)',
                                border: InputBorder.none,
                                contentPadding: const EdgeInsets.all(16),
